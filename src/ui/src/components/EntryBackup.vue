    ...
    >
      <div class="hero-body">
        <div class="container">
          ...
          <div v-if="isError">
            <p class="my-4">
              <span class="has-text-danger px-1">{{ errorMessage }}</span>
            </p>
          </div>
          ...
          <div v-if="isPreparingFiles">
            <p class="my-4">
              ...
              <span
                >{{ progress.entryCount }} entry iceren yedekleme dosyaları
                hazırlanıyor</span
              >
            </p>
          ...
          <div v-if="isDownloadReady">
            <p class="my-3">
              ...
              <span class="px-1"
                ...
                <b>{{ progress.entryCount }}</b> entry yedeklendi.
              </span>
            </p>
            ...
            <p class="my-3">
              ...
              <span class="px-1"
                >pdf, html, json ya da hepsini içeren zip dosyasını aşağıdaki
                bağlantılardan kaydedebilirsiniz.
              </span>
            </p>
            ...
            <p class="buttons my-4">
              <a
                ...
                :href="`${serviceUrl}/download/${sid}`"
                target="_blank"
                :download="`entry-backup_${username}.zip`"
                >{{ renderResult.zipFilename }}</a
              >
              <a
                class="button is-info is-normal"
                :href="`${serviceUrl}/download/${sid}/${renderResult.pdfFilename}`"
                target="_blank"
                :download="`entry-backup_${username}.pdf`"
                >{{ renderResult.pdfFilename }}</a
              >
              <a
                class="button is-info is-normal"
                :href="`${serviceUrl}/download/${sid}/${renderResult.htmlFilename}`"
                target="_blank"
                :download="`entry-backup_${username}.html`"
                >{{ renderResult.htmlFilename }}</a
              >
              <a
                class="button is-info is-normal"
                :href="`${serviceUrl}/download/${sid}/${renderResult.jsonFilename}`"
                target="_blank"
                :download="`entry-backup_${username}.json`"
                >{{ renderResult.jsonFilename }}</a
              >
            </p>
          </div>
        </div>
